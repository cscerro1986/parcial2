        ...
        do
            ...
            if(!getString(msg,msgError,minSize,maxSize,&reintentos,bufferStr)) //==0 sin errores !0
            {
                if(isValidNumber(bufferStr)==1)
                {
                    ...
                    {
                        printf("%s 3 Numero fuera de rango",msgError);
                    }
...
int utn_getFloat(char* msg, char* msgError, int minSize, int maxSize, int min, int max, int reintentos, float* input)
{
    ...
    int retorno=-1;
    char bufferStr[maxSize];
    printf("ingreso a la funcion\n\n");

    if(msg!=NULL && msgError!=NULL && minSize<maxSize && min<max && reintentos>=0 && input!=NULL)
    {
        printf("antes de entrar al do");
        do
        {
            printf("Entro al do");
            if(!getString(msg,msgError,minSize,maxSize,&reintentos,bufferStr)) //==0 sin errores !0
            {
                if(isValidFloatNumber(bufferStr)==1)
                {
                    ...
                    {
                        printf("%s 3, Numero fuera de rango");
                    }
...
int utn_getUnsignedIntCorregida(char* msg,char* msgError,int minSize,int maxSize,int min,int max,int reintentos,int* input)
{
    int retorno=-1;
    char bufferStr[maxSize];
    int numero;
        ...
        do
        {
            if(!getString(msg,msgError,minSize,maxSize,&reintentos,bufferStr)) //==0 sin errores !0
            {
                ...
                if(isValidNumber(bufferStr)==1)
                {
                    ...
                    {
                       printf("%s 2, numero fuera de rango\n",msgError);
                ...
                {
                     printf("%s 3, el numero no es valido\n",msgError);
                     reintentos--;
                }
            }
        }
